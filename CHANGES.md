...
## 미해결 이슈(백로그) — 2026-07-16 기록

| ID | 증상 | 원인·상태 |
|----|------|-----------|
| I1 | **배포 시 진행 중 생성 사망** — 재배포(인스턴스 교체)가 백그라운드 생성 스레드를 죽여 배치 항목이 `generating`으로 고착되고, 업로드 쿼터가 환불되지 않음(스레드 킬은 예외 경로가 아님). 검증 중 3회 실측 | 미해결. 후보: 기동 시 `generating` 상태 청소(+쿼터 환불), 생성을 프로세스 밖 큐로 이동, graceful shutdown 훅. 운영 수칙: 생성 진행 중 push 금지, 배포 SUCCESS 확인 후 작업 |
| I2 | **strategist 브리프 경유 날조 통로** — `brief_to_directive(brief)`가 asset.note에 합쳐진 뒤 `gen_source`(날조 대조 기준)가 되므로, 브리프(LLM 산출물)가 만든 수치·표현은 날조 검사를 통과함 | 미해결. 후보: gen_source를 브리프 합류 '이전' 스냅샷으로 저장, 또는 브리프 자체에 수치 생성 금지 지시+브리프 산출 수치 별도 검증 |
