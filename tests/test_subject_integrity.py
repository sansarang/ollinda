...
def test_script_gate_does_not_block_particle_forms():
    """2026-08-16 실물 과잉차단: '후에도'가 세 영상 연속 막혀 재시도를 다 쓰고 폴백으로 떨어졌다.
    본문에 '후'가 16회 있는데 2자 어간 '후에'를 찾다가 못 찾은 것 — '에도'는 조사인데
    어간으로 오인했다. 조사를 떼고 한 번 더 봐야 한다(한국어 규칙, 업종어 0).

    ★ 위험 비대칭은 유지돼야 한다 — 지어낸 고유명사는 조사를 떼도 본문에 없어 그대로 걸린다.
    """
    from app.generators.video import _fact_guard as _ur
    src = "시공 후 유리막코팅을 했습니다. 테슬라 모델Y 작업."
    # 조사 결합·의문사·의문 종결어미 — 전부 사람 말이지 날조가 아니다
    for ok in ("시공 후에도 그대로입니다.", "뭐부터 보시면 좋을까요?", "어디부터 하실지 정하세요.",
               "모델Y를 다시 봤습니다.", "코팅으로 마감했습니다.", "얼마나 걸릴까요?"):
        assert not _ur(ok, src), f"정상 표현을 막았다: {ok} → {_ur(ok, src)}"
