...
def _seller_longtail_candidates(t) -> list:
    ...
    for c in db.recent_inventory_context(t.id, limit=6):
        ...
        if c.get("year"):
            years.append(c["year"])
    # 치환 규칙은 longtail.combos 한 곳에만 산다(정찰 씨앗과 같은 함수).
    return _lt.combos(t, attrs, years=years)
