...
def ingest_upload(tenant: Tenant, files: list[tuple[bytes, str]], note: str,
    ...
    # 📦 매물/상품 컨텍스트 저장(셀러·병행) — 오토큐 롱테일·태그 정합 재료. 업종 스키마 attribute_axes로
    #    핵심 속성(1축=차종·향·모델명)·분류(이후 축=차급·용량)를 폼·vision 확정값에서만 추출(날조·차량 하드코딩 0).
    try:
        _bt2 = (getattr(tenant, "biz_type", "local") or "local")
        if _bt2 in ("seller", "hybrid"):
            import re as _rc
    ...
    try:
        ...
        if learn:
            ...
            # ★ 2026-08-16 실사고: 이 지시문이 **소재를 갈아치웠다.**
            #   '제네시스 GV80 신차패키지'가 (잘못) 오른 키워드로 뽑혀 "제목·첫문장·본문에 더 강하게
            #   반영하라"로 들어갔고, 모델이 그대로 따라 **테슬라 사진 글을 GV80 글로** 썼다.
            #   학습 키워드는 소재를 이길 수 없다 — 소재는 사진과 입력이 정한다(헌법: 소재 정합).
            asset.note += (
                f"\n[성과 학습 — 참고 키워드] 아래는 과거 성과가 있었던 검색어다: {kwlist}\n"
                "★ 단, 이 글의 **소재는 이번 사진·입력이 정한다.** 위 검색어는 소재를 바꾸는 근거가 아니다.\n"
                "  · 이번 소재와 어긋나면 **무시하라**(억지로 끼워 넣지 마라).\n"
                "  · 사진에 없는 차종·제품·모델명을 위 검색어 때문에 쓰면 안 된다 — 그건 날조다.\n"
                "  · 소재와 결이 맞을 때만 표현을 자연스럽게 맞추는 정도로 쓴다.")
    except Exception:
        pass
...
def request_video_bundle(tenant: Tenant, asset_id: str, want: set[str]) -> tuple[bool, str]:
    ...
    from app import llm as _llmg           # 💳 크레딧 소진이면 즉시 중지(헛 렌더·원시 오류 방지)
    if _llmg.credit_out():
        return False, _llmg.CREDIT_MSG
    pieces = db.get_set_pieces(asset_id)
    blog = next((p for p in pieces if p.kind == ContentKind.BLOG), None)
    if not blog:
        return False, "글이 아직 없는 콘텐츠예요"
