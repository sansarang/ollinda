...
def _analyze_gap(tenant, pub: dict, kw: str, cause: str, existing: list[str]) -> str:
    ...
    try:
        ...
        from app import llm
        # 🔄 2026-08-18 Solar 전환(사장님 지시) — 순위가 왜 밀렸는지 추론해 '다음 글의 교훈'을 만든다.
        #   틀린 교훈은 모든 가게의 다음 글에 박히므로 effort를 낮추지 않는다(analysis=medium).
        #   전에는 클로드 직행이라 크레딧이 0이면 교훈이 아예 안 쌓였다 — 학습이 멈춘다.
        v = llm.call_task(
            "analysis",
    ...
    if len(active) < _GL_MAX_ACTIVE:
        for k, n in cnt.most_common():
            ...
            try:
                ...
                # 🔄 2026-08-18 Solar 전환 — 여러 가게에 반복된 감점에서 '전체 생성 지시'를 뽑는다.
                #   한 문장이 모든 tenant의 글에 적용되므로 effort를 낮추지 않는다(analysis=medium).
                lesson = (_llm.call_task(
                    "analysis",
