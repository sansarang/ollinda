...
def diagnose_rank(industry: str, region: str, name: str, addr: str = "") -> dict:
    ...
    return {
        ...
        # ★ 블로그를 못 찾았으면 순위를 말하지 않는다(2026-08-14 캡쳐 검토에서 발견).
        #   제목은 "글이 없어요"라고 해놓고 표에서는 '상위 5위 밖'이라 순위를 말하고 있었다 —
        #   읽는 사람에겐 "내 글이 5위 밖"으로 읽힌다. 없는 글의 순위를 있는 것처럼 말한 셈.
        #   글이 없을 때 저 줄들은 순위표가 아니라 '손님이 이렇게 찾고 있다'는 기회 목록이다.
        "no_blog": not bool(blog_id),
        # 5위까지만 스캔한다 — '미노출'이라 하면 아예 없는 것처럼 오해된다(2026-08-12 사장님 지적).
        "miss_label": "상위 5위 밖" if blog_id else "아직 내 글 없음",
    }
