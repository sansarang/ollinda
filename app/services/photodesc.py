...
def write_captions(descs: list, anchors=(), shop: str = "", kw: str = "",
    ...
    prompt = (
        ...
        "관찰 기록이 **대상(피사체)을 추측해 부른 자리**에만 이 실값을 써라.\n"
        "   ★ 실값을 다른 자리에 끼워 넣지 마라. 도구·재료·부위 이름으로 바꿔 쓰는 것은 날조다.\n"
        "   ★ 그 실값이 무엇인지(대상의 이름인지, 재료 등급인지) 확실하지 않으면 아예 쓰지 마라.\n"
        "   관찰 기록에 도구가 '빨간색 스퀴지'로 나오면 그대로 '시공 도구'라고 써라.\n"
