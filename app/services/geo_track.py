...
# ★ 2026-08-07 실측: '지켜라' 지시만으로는 G1 5/6·G2·G3에서 떨어졌다(전 업종 공통 게이트).
#   출력 직전에 스스로 세어보게 한다. 생성(info_prompt)과 재생성(revise_piece)이 이 한 덩이를
#   같이 쓴다 — 재생성만 이 계약을 모르면 수정할 때마다 구조가 무너진다(같은 날 실측:
#   honesty 지시만 실은 재생성 후 최종 G1 1/7).
SELF_CHECK = (
    "[출력 전 자가 점검 — 하나라도 아니면 고쳐서 출력하라]\n"
    "① 제목과 **모든** ##·### 소제목이 물음표로 끝나는가? "
    "('~정리', '~방법' 같은 평서형 소제목이 하나라도 있으면 실패다. FAQ·요약 블록은 예외)\n"
    "② 각 ## 바로 다음 첫 문장이 **평서문 정답**인가? "
    "(질문형 소제목 뒤에 또 물음표 문장을 두면 실패. '~입니다/~합니다'로 끝나는 완결 답이어야 한다)\n"
    "③ 본문에 단위 붙은 수치가 **3개 이상**인가? "
    "(예: 3년, 15%, 12만원, 40분, 2회 — 입력·사진분석에 근거한 실값만. 없으면 지어내지 말고 "
    "대신 아는 실값을 더 찾아 써라)\n"
    "④ '## 자주 묻는 질문'(3쌍)과 '## 3줄 요약'이 둘 다 있는가?\n"
    "⑤ 각 단락이 400자 이하인가?\n"
)
...
def info_prompt(tenant, industry: str, region: str, kw: str, angle: str,
    ...
    return (
        ...
        # 이미지 검색 유입(2026-08-01) — 네이버 이미지탭은 '사진 주변 문맥'을 본다(전 업종 공통)
        f"[이미지 검색 노출] 각 [사진N] 바로 앞·뒤 문장에 그 사진이 무엇인지 알려주는 구체 설명을 두고, "
        f"최소 2곳에서는 '{kw}'의 자연 변형이나 그 사진의 핵심 대상 이름이 그 문장에 자연스럽게 들어가게 하라. "
        "키워드 나열·억지 삽입 금지(저품질 신호) — 읽는 사람에게 필요한 설명이 먼저다.\n\n"
        + SELF_CHECK + "\n"
