...
def prompt_rule(keywords=None, core: str = "") -> str:
    ...
    lines += [
        ...
        # 🧹 통폐합 3-4(2026-08-16): 따로 서 있던 [본문 구조 — 적게, 두껍게]에서
        #   고유 규칙 둘만 여기로 옮겼다. 나머지 세 줄(두꺼운 문단·독립 문단·흩뿌리기 금지)은
        #   위에 이미 같은 말로 있었다 — 같은 지시를 두 블록이 반복하면 새 지시가 묻힌다.
        #   근거: 남의 상위글 28개 키워드 해부 — 소제목 중간값 2개(우리는 6~9개), 표 0%.
        "· 본문 소제목은 **2~3개만** 만들어라. 많이 쪼개지 마라"
        "(상위 글 실측 중간값 2개 — 잘게 쪼갤수록 각 덩어리가 얕아져 뽑아갈 단위가 사라진다).",
