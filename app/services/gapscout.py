...
MAP_TTL_DAYS = 14
# ★ 2026-08-12 사장님 지적 + 실측: '빈자리 정찰'이 실제로는 '검색량 순위표'로 동작했다.
#   실측 점수(문서수 반영): 썬팅가격(4,330회/문서 50만) 5.71 > 신차썬팅(100회/문서 2만) 4.42.
#   경쟁 약도 기여가 0.02~0.42로 미미해 큰 키워드가 항상 이겼고, 소상공인이 못 이기는 판에
#   글을 쓰게 됐다. → 경쟁 약도를 실제로 무겁게 하고, 동네 실수요(월 30~80회)를 살린다.
MIN_VOLUME = 30           # 소상공인 실수요 하한 — '부산 기장 중고차'(월 80회)는 그 동네에서
                          #   매달 80명이 차를 찾는다는 뜻이다. 100 관문은 이런 알짜를 잘랐다.
                          #   ※ 지역 표면 허용(seo.REGION_MIN_VOLUME=100)은 그대로 — 다른 잣대다.
STALE_TOP_DAYS = 365      # 상위 글이 이만큼 오래됐으면 '경쟁 약함' 신호(1년+)
STALE_BONUS = 0.5         # 그 신호의 가산점(고정) — weak_comp를 곱하지 않는다(보조 신호)
...
WEIGHTS = {
    "volume_log": 0.6,        # 수요(월검색량, 로그) — ★ 1.0→0.6(2026-08-12). 로그를 써도
                              #   9.7만회 키워드가 문서 40만 판을 이기고 1위가 됐다. 소상공인에겐
                              #   '얼마나 크냐'보다 '이길 수 있냐'가 먼저다(MIN_VOLUME이 하한을 지킨다).
    "surface": 2.0,           # 지면 존재(블로그 자리가 실제로 뜨는가) — 없으면 써도 안 보인다
    "weak_comp": 4.0,         # 경쟁 약도(문서 수 적음 · 상위 글 낡음) — ★ 1.5→4.0(2026-08-12)
                              #   '이길 수 있는 판'이 '큰 판'을 이기게 하는 핵심 가중치.
}
...
def _score(volume: int, has_surface: bool, doc_count: int, top_age: int) -> float:
    ...
    if isinstance(doc_count, int) and doc_count > 0:
        # ★ 2026-08-12: 예전엔 max(0, …)로 하한을 잘라 '문서 50만 건 판'도 벌점이 0이었다.
        #   경쟁이 심하면 실제로 감점돼야 '이길 수 있는 판'이 위로 온다. 하한을 풀되
        #   -1.0으로 한정해 대형 키워드가 영원히 배제되지는 않게 한다(보조 목표로는 남는다).
        weak += max(-1.0, 1.0 - math.log10(max(10, doc_count)) / DOC_LOG_DIV)
    stale_bonus = 0.0
    if isinstance(top_age, int) and top_age >= STALE_TOP_DAYS:
        # ★ '상위 글이 낡음'은 보조 신호다 — weak_comp(4.0)를 곱하면 +2.0이 되어
        #   문서 60만 건짜리 대형 키워드가 이 보너스만으로 1위를 유지했다(2026-08-12 실측).
        #   경쟁 약도(문서 수)와 분리해 고정 소액으로 준다.
        stale_bonus = STALE_BONUS
    return round(s + WEIGHTS["weak_comp"] * weak + stale_bonus, 3)
