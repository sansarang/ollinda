...
def outsider_mentions(body: str, terms: list, material: str) -> list:
    """판의 언어 중 **재료에 없는데 글에 등장한 말** — 검토 대상(경보가 아니다).

    ★ 2026-08-17 실물 확인 — 여기 잡히는 것이 전부 위험한 것은 아니다.
      '전면유리'·'눈부심'처럼 **일반 주제어**가 잡히면 오히려 커버율이 올랐다는 신호다
      (재료에 없던 손님 관심사를 다뤘다는 뜻). 위험한 것은 남의 제품명이다.
      코드는 그 둘을 구분하지 못한다(브랜드 판별은 불가능함이 실측으로 드러났다).
      그래서 이 목록은 **사람이 훑어보라는 것**이지 자동 차단이 아니다.
