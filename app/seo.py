...
def _volume_boost_cached(hints_key: str) -> tuple:
    """네이버 검색광고 API로 실검색량 스윗스팟(500~5,000) 키워드(캐시). 무키/실패 시 빈 튜플."""
    try:
        from app.services import searchad
        if not searchad.configured():
            return tuple()
        return tuple(searchad.sweet_spot_keywords([h for h in hints_key.split("|") if h]))
    except Exception:
        return tuple()
...
def target_keywords(industry_name: str, region: str, note: str = "", limit: int = 10,
    ...
    for k in kws:
        if k not in seen:
            seen.add(k); out.append(k)
    return _apply_volume(out, limit, hints=([ind] if ind else None))
