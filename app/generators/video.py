...
#   앞뒤가 끊긴 말이 된다. ★ 마무리 줄도 예외가 아니다(2026-08-02 수정) — 아웃트로가
#   '본문에서 확인하세요'를 쓰고 있었고, 그 영상이 네이버 클립 지면으로도 나간다.
SELFREF_PATTERNS = (
...
FEAR_PATTERNS = (
    ...
    # ★ 조건부 위협형(2026-08-03 실측) — '안 하면 후회한다'는 전형적 불안 마케팅인데
    #   기존 목록이 낱말만 봐서 통과했다. 실제 문장: "신차 뽑자마자 이 작업 안 하면,
    #   6개월 뒤에 후회합니다" / "가격표만 보고 결정하기엔 불안하셨을 겁니다".
    #   ※ '후회 없는' 같은 긍정형은 잡지 않는다(뒤에 부정어가 오는 형태만).
    r"안\s?하면[^.]{0,20}(후회|큰일|낭패|늦)", r"놓치면[^.]{0,15}(후회|손해)",
    r"후회(합니다|하십니다|해요|하게 됩니다|할 수)", r"불안하셨",
)
RIVAL_PATTERNS = (
...
_SPOKEN_FUNC = {
                # ★ 2026-08-16 실물 과잉차단: '뭐부터'가 막혀 재시도를 소진하고 폴백으로 떨어졌다.
                #   의문사·대명사·정도부사는 **날조 대상이 아니다**(고유명사가 아니라 문법어).
                #   본문에 그 말이 없다고 막으면 사람 말투가 통째로 금지된다.
                "뭐", "무엇", "어디", "언제", "누구", "어느", "어떤", "얼마",
                "이거", "그거", "저거", "이런", "그런", "저런", "여기", "거기", "저기",
                "조금", "조금씩", "훨씬", "제일", "가장", "특히", "역시", "아무", "전부", "모두",
                "오늘", "지금", "바로", "이렇게", "정말", "함께", "그리고", "그래서", "그럼",
...
def _fact_guard(line: str, source: str) -> str:
    ...
    _PRED = _rg.compile(
        ...
        # ★ 2026-08-16: 의문 종결어미가 빠져 '좋을까요'가 날조로 잡혔다.
        #   고유명사가 이 꼬리로 끝나는 일은 없다 — 넣어도 위험 비대칭이 유지된다.
        r"까요|나요|가요|을까요|ㄹ까요|던가요|는가요)$")
    # 조사·보조사 꼬리(언어 규칙만) — '후에도'처럼 1자 어근 + 조사인 말을 어간 오인하지 않게.
    _JOSA_TAIL_V = _rg.compile(
        r"(에서도|에게도|으로도|이라도|까지도|부터도|에도|에는|에서|으로|이라|까지|부터|"
        r"에게|한테|보다|처럼|만큼|이나|마다|조차|대로|밖에|보고|와도|과도|도|를|을|은|는|이|가|의|에|와|과|로|만)$")
    ...
    for tok in _rg.findall(r"[가-힣]{3,}", line):
        ...
        if tok[:2] in _srcflat:                        # 2자 어간이 본문에 실존 → 통과(활용·조사 결합 허용)
            continue
        # ★ 2026-08-16 실물 과잉차단: '후에도'가 세 영상 연속 막혔다.
        #   본문에 '후'가 16회 있는데 2자 어간 '후에'를 찾다가 못 찾은 것이다 —
        #   '에도'는 조사인데 어간으로 오인했다. 조사·어미를 떼고 한 번 더 본다.
        #   지어낸 고유명사는 조사를 떼도 본문에 없어서 그대로 걸린다(위험 비대칭 유지).
        _stem = _JOSA_TAIL_V.sub("", tok)
        if _stem and (_stem in _srcflat or _stem in _SPOKEN_FUNC):
            continue                                  # 조사를 뗀 어근이 본문에 있거나 기능어면 통과
...
def _script_from_body(body: str, n: int, kw_nat: str, source: str, tone: str = "info",
    ...
    for attempt in (1, 2):
        ...
        #   본문에 없는 말은 고쳐 쓸 수 없다 — **빼라**고 명시해야 한다.
        feedback = (f"\n\n[재작성 — 직전 대본이 검증에서 차단됨: {bad}]\n"
                    "★ 지적된 표현은 **본문에 없는 말**이다. 바꿔 쓰지 말고 **그 말을 빼라** — "
                    "그 자리에는 본문에 실제로 있는 단어만 써라. 없으면 그 씬을 통째로 다른 내용으로 바꿔라.\n"
                    "같은 지적이 두 번 나오면 그 씬은 버리고 본문의 다른 대목으로 채워라.")
    return None
...
class ShortVideoGenerator(Generator):
    ...
    def _naver_video(self, tenant, asset, vid_imgs, kws, strat, out_dir):
        ...
        구성: [키워드 질문형 오프닝] → [핵심 답 3(글 소제목 축약)] → [사진 장면+본문 발췌 캡션]
              → [마무리: 가게명+지역+자립형 행동 유도]. 감성 훅·밈 금지 — 검색어에 답하는 구조.
              마무리는 글을 가리키지 않는다 — 같은 영상이 클립 지면으로도 나가기 때문이다.
        ...
        if _script_mode:
            ...
            if len(_pairs_l) >= 3:
                ...
                _photo_locked = True
        # 클로징 — 글 CTA '사실' 기반 선택(본문에 근거 있는 패턴만).
        #   ★ 2026-08-02 사장님 지적 2건을 함께 고친다.
        #   ① '본문에서 확인하세요'는 자기참조다. 자막에서 글을 가리키지 않기로 한 규칙을
        #      마무리 줄만 안 지키고 있었다(게이트를 안 타는 경로였다). 블로그에 붙으면 말이 되지만
        #      같은 마무리가 네이버 클립 지면으로도 나간다 — 거기선 '본문'이 없다.
        #      → 영상만 봐도 말이 되는 자립형 문구로 바꾼다(지면 분기 없이 양쪽에서 성립).
        #   ② '중고'가 업종 문자열로 박혀 있었다(업종 중립 위반). 근거는 업종명이 아니라
        #      본문이 실제로 무엇을 다루는지에서 온다.
        _has_doc = any(k in body for k in ("성능점검", "서류", "점검기록부", "보증서", "증명서"))
        _has_visit = any(k in body for k in ("예약", "방문", "오시면", "오세요", "매장"))
        if _has_doc:
            _cta_line = "서류는 원본 그대로 보여드립니다"
        elif _has_visit:
            _cta_line = "실물 확인은 예약 한 번이면 됩니다"
        else:
            _cta_line = "궁금한 점은 편하게 물어보세요"
        if _SELFREF.search(_cta_line):             # 규칙이 바뀌어도 자기참조가 새어나가지 않게
            _cta_line = "궁금한 점은 편하게 물어보세요"
